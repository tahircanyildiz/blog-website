 ...
 * @param {Object} props
 * @param {Object} props.aboutData - Hakkımda verileri
 * @param {Object} props.contactInfo - İletişim bilgileri
 */
function AboutSection({ aboutData, contactInfo }) {
  ...
  // PDF oluşturma fonksiyonu (About.jsx'ten aynen alındı)
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4 sm:space-y-6">
              {aboutData.experiences.map((exp, index) => {
                const expAnimation = useScrollAnimation({ threshold: 0.2 });
                return (
                  <div
                    key={index}
                    ...
                    className={`bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-6 border-l-4 border-[#40916c] hover:shadow-2xl hover:border-[#2d6a4f] transition-all duration-300 group ${
                      expAnimation.isVisible ? 'animate-fadeInUp' : 'opacity-0'
                    }`}
