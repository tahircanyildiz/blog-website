                ...
                >
                  <div className="bg-white rounded-3xl shadow-md hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 hover:border-[#95d5b2]">
                    <div className="md:flex">
                      {/* Sol - gradient banner */}
                      <div className="md:w-2/5 bg-gradient-to-br from-[#40916c] via-[#2d6a4f] to-[#1b4332] p-6 sm:p-8 md:p-10 flex flex-col justify-center relative overflow-hidden">
            ...
            {otherBlogs.length > 0 && (
              <div className="grid gap-5 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
                {otherBlogs.map((blog, index) => {
                  const blogAnimation = useScrollAnimation({ threshold: 0.2 });
                  return (
                    <div
                      key={blog._id}
                      ref={blogAnimation.ref}
                      className={blogAnimation.isVisible ? 'animate-fadeInUp' : 'opacity-0'}
                      style={{ animationDelay: `${index * 0.1}s` }}
                    >
                      <Link
                        to={`/blog/${blog.slug}`}
                        className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300
                               transform hover:-translate-y-1 overflow-hidden group border border-gray-100 hover:border-[#95d5b2] flex flex-col h-full"
                      >
                        {/* Üst gradient çizgi */}
